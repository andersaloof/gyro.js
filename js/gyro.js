...
(function() {
 ...
 (function (root, factory) {
	...
	gyro.step = function(timestamp) {
		if (!animation_start) animation_start = timestamp;
		if (timestamp - animation_start >= gyro.frequency) {
			animation_start = timestamp;
			callback(measurements);
		}
		animation_id = window.requestAnimationFrame(gyro.step);
	};
	
	gyro.startTracking = function(cbk) {
		callback = cbk;
		animation_id = window.requestAnimationFrame(gyro.step);
	};
